...
var _ = Describe("restore/data tests", func() {
	...
	Describe("GenerateResizeMaps", func() {
		DescribeTable("Validate various flag combinations that are required or exclusive",
			func(originSize int, destSize int, expectedBatchMap string, expectedContentMap string) {
				batchMap, contentMap := restore.GenerateResizeMaps(originSize, destSize)

				Expect(batchMapToString(batchMap)).To(Equal(expectedBatchMap))
				Expect(contentMapToString(contentMap)).To(Equal(expectedContentMap))

			},
			Entry("5->2", 5, 2, "0: {0:0, 1:1}; 1: {2:0, 3:1}; 2: {4:0}", "{0:0, 1:1, 2:0, 3:1, 4:0}"),
			Entry("3->5", 3, 5, "0: {0:0, 1:1, 2:2}", "{0:0, 1:1, 2:2}"),
			Entry("4->4", 4, 4, "0: {0:0, 1:1, 2:2, 3:3}", "{0:0, 1:1, 2:2, 3:3}"),
		)
	})
})
